...
export const id = 'commerzbank_de' as const;
...
export function detect(text: string): { hit: boolean; confidence: number } {
  ...
  for (const line of lines) {
    ...
    if (header.includes('umsatzineur')) {
      continue;
    }
    if (header.includes('umsatzineur')) {
      continue;
...
export function parse(fileBuffer: Buffer): ParseResult {
  ...
  if (!detection.hit) {
    return {
      ...
      confidence: 0,
      rows: [],
      warnings: ['Header nicht als Commerzbank-Export erkannt.'],
  ...
  records.forEach((record, index) => {
    ...
    const type = get(record, ['Vorgang', 'Buchungstext']).trim();
    ...
    const rawText = [type, memo].filter(Boolean).join(' ') || 'Commerzbank';

    const raw: Record<string, unknown> = { __source: 'csv_commerzbank_de', __index: index };
